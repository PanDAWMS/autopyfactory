#!/bin/env python
#
# AutoPyfactory condor pool status plugin for Condor
#
...
class CondorPoolBatchStatusPlugin(threading.Thread, BatchStatusInterface):
    '''
    -----------------------------------------------------------------------
    ...
    def __init__(self, apfqueue, **kw):
        ...
        try:
            ...
            self.condorpool = self.apfqueue.qcl.generic_get(self.apfqname, 'batchstatus.condor.condorpool', 'get', logger=self.log)
            ...
            self.activity2info = {'Busy': 'running',
                                  'Idle': 'running',
                                  'Retiring': 'done'}
    ...
    def _querycondor(self):
        ...
        condor_status -pool gridtest03.racf.bnl.gov:29660 -format "Name=%s " Name -format "Activity=%s " Activity -format "State=%s " State -format "IP=%s\n" MyAddress -xml

        '''
        self.log.debug('_querycondor: Starting.')
        querycmd = "condor_status"

        # adding the pool
        querycmd += ' -pool %s' %self.condorpool

        # verbatim input options from the queues config file
        if self.queryargs:
            querycmd += ' ' + self.queryargs
 
        querycmd += " -format ' Name=%s' Name"
        querycmd += " -format ' Activity=%s' Activity"
        querycmd += " -format ' State=%s' State"
        querycmd += " -format ' IP=%s' MyAddress"
        querycmd += " -xml"

        self.log.debug('_querycondor: Querying cmd = %s' %querycmd.replace('\n','\\n'))

        # Run and time condor_status
        # FIXME
        # If condor_status can take a long time, we should wrap this in a fully protected 
        # timed command
    ...
    def _parseoutput(self, output):
        '''
        parses XML output of condor_status command with an arbitrary number of attribute -format arguments,
        and creates a Python List of Dictionaries of them. 
        
        Input:

            CTYPE classads SYSTEM "classads.dtd">
            <classads>
            <c>
                <a n="MyType"><s>Machine</s></a>
                <a n="TargetType"><s>Job</s></a>
                <a n="Name"><s>server-100.novalocal</s></a>
                <a n="MyAddress"><s>&lt;172.10.8.78:21825?CCBID=130.199.185.191:29660#168022&amp;PrivNet=localdomain&gt;</s></a>
            ...
            <c>
                <a n="MyType"><s>Machine</s></a>
                <a n="TargetType"><s>Job</s></a>
                <a n="Name"><s>server-101.novalocal</s></a>
                <a n="MyAddress"><s>&lt;172.10.8.79:27738?CCBID=130.199.185.191:29660#168026&amp;PrivNet=localdomain&gt;</s></a>
                <a n="State"><s>Claimed</s></a>
                <a n="Activity"><s>Busy</s></a>
                <a n="CurrentTime"><e>time()</e></a>
            </c>
