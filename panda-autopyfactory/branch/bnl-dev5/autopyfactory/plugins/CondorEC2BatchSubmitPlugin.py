...
class BatchSubmitPlugin(BatchSubmitInterface):
        ...
        def __init__(self, wmsqueue):
                ...
                self.environ = None
                if self.qcl.has_option(self.apfqueue, 'batchsubmit.condorec2.environ'):
                        self.environ = self.qcl.get(self.apfqueue, 'batchsubmit.condorec2.environ')

                self.condor_attributes = None
                if self.qcl.has_option(self.apfqueue, 'batchsubmit.condorec2.condor_attributes'):
                        self.condor_attributes = self.qcl.get(self.apfqueue, 'batchsubmit.condorec2.condor_attributes')
        ...
        def __createJSDFile(self):
                ...
                self.JSD.add('grid_resource=ec2 %s' % self.jdl) 
