...
Those processes that are shared between more than one APFQueue,
as querying the WMS service or a Condor pool,
are performed in an unique thread (implemented as a Singleton design pattern),
and their output is shared by different other modules. 
This mechanism highly reduces the load on those services and increases the reliability and performance of the whole system.
